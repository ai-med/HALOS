...
from models.metrics import SegMetric, CLFMetric, GallbladderOnly
...
def train_fold(config, cv_index, path_run, dataPreparer):
    ...
    prefix = "HALOS_kidney_" + str(config["fusion_loca"]) + "_loca" + str(
    ...
    unet = Final_UNet(
        ...
        fusion_loca=fusion_loca, fusion_squeeze_factor=fusion_squeeze_factor, fusion_feature_dim=1,
    ).to(device)
    ...
    # set metrics
    seg_metric_train = SegMetric(organ="gallbladder", mode="train", save_individual_result=False)
    seg_metric_val = SegMetric(organ="gallbladder", mode="train", save_individual_result=False)
    clf_metric_train = CLFMetric(num_classes=2)
    clf_metric_val = CLFMetric(num_classes=2)
    fpr_metric_val = GallbladderOnly(save_individual_result=False)
        ...
        for counter, train_batch_data in enumerate(loader_train):
            ...
            with torch.cuda.amp.autocast():
                ...
                if config['fusion_input'] == 'clf':
                    ...
                    enc_outputs.update({"encoder": False, "clf_results": softmax(clf_outputs[:num_seg_img])[:, -1],
            ...
            with torch.cuda.amp.autocast():
                with torch.no_grad():
                    ...
                    for val_counter, val_batch_data in enumerate(loader_val):
                        ...
                        val_enc_outputs.update({"encoder": False, "clf_results": softmax(val_clf_outputs)[:, -1]})
